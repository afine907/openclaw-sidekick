...
class DiagnoseTool {
  ...
  async diagnoseNetwork(apiEndpoint) {
    ...
    try {
      ...
      results.api = response.ok
        ? `OK (${response.status}) - ${(performance.now() - start).toFixed(0)}ms`
  ...
  render() {
    ...
    // 绑定事件
    document
      .getElementById("diagnose-run")
      ?.addEventListener("click", () => this.runDiagnose());
    document
      .getElementById("diagnose-export")
      ?.addEventListener("click", () => this.exportAndDownload());
    document
      .getElementById("diagnose-clear")
      ?.addEventListener("click", () => this.clearLogs());
    document
      .getElementById("diagnose-toggle")
