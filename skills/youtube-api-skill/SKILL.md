...
Only the endpoints listed in the API Reference section below are supported. Maton proxies requests to `www.googleapis.com` and automatically injects your OAuth token.
...
```bash
curl -s -X GET "https://api.maton.ai/youtube/youtube/v3/search?part=snippet&q=machine+learning&type=video&maxResults=10&order=viewCount" -H "Authorization: Bearer $MATON_API_KEY"
```
...
```bash
curl -s -X GET "https://api.maton.ai/youtube/youtube/v3/videos?part=snippet,statistics&id=dQw4w9WgXcQ" -H "Authorization: Bearer $MATON_API_KEY"
```
